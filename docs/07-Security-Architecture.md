...
| 5 | **Session Hijacking** | An attacker obtains a valid session token (network sniffing, XSS, shared device) and replays it. | Low | High | `HttpOnly`/`Secure`/`SameSite=Strict` cookies (Section 5); mandatory HTTPS/TLS (Section 13 of [08-Security-Controls.md](08-Security-Controls.md#13-application-security)); short access-token TTL (15 min) limits the exploitation window. |
| 6 | **CSRF** | A malicious site tricks an authenticated user's browser into submitting a state-changing request using their live session cookie. | Medium | Medium | `SameSite=Strict` cookies + double-submit CSRF token on all state-changing requests (Section 9 of [03-Security.md](03-Security.md#9-csrf), extended in SDR-007). |
...
Foundational mechanism: stateless JWT access token + rotating opaque refresh token (ADR-0003), delivered via `HttpOnly`/`Secure`/`SameSite=Strict` cookies (Section 5). This section walks every flow step by step; component interaction diagrams already exist in [02-Architecture.md §8](02-Architecture.md#8-authentication-flow) and are not redrawn here.
...
6. On success: reset `failed_attempts` to zero; issue a new access token (15 min, JWT) and refresh token (7 days, opaque, hashed and stored server-side); set both as `HttpOnly`/`Secure`/`SameSite=Strict` cookies; also issue the CSRF token cookie (Section 9 of [03-Security.md](03-Security.md#9-csrf)) needed for subsequent state-changing calls.
...
|---|---|---|
| `access_token` | `HttpOnly`, `Secure`, `SameSite=Strict`, `Path=/api`, no `Max-Age` beyond token TTL | Carries the JWT; inaccessible to JavaScript (closes XSS-token-theft, threat #7/#5). |
| `refresh_token` | `HttpOnly`, `Secure`, `SameSite=Strict`, `Path=/api/v1/auth/refresh` (scoped narrowly — sent only to the one endpoint that needs it, minimizing exposure on every other request) | Carries the opaque refresh token. |
| `csrf_token` | `Secure`, `SameSite=Strict`, **not** `HttpOnly` (must be readable by the SPA's JS to echo into the `X-CSRF-Token` header, Section 9 of [03-Security.md](03-Security.md#9-csrf)) | Double-submit CSRF defense. |
