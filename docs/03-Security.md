...
- **CSRF token (double-submit)** for state-changing (`POST`/`PUT`/`PATCH`/`DELETE`) requests as defense in depth, since `SameSite` support/behavior can vary by browser/proxy configuration: a non-`HttpOnly` CSRF token cookie is set at login, and the value is echoed by the SPA in a custom request header (`X-CSRF-Token`); Spring Security validates the header matches the cookie. A cross-site request cannot obtain the current value to construct the matching header. Since the SPA and API are on different subdomains, the SPA's own JS cannot read that cookie either (`document.cookie` is scoped to the setting origin, not the page's) — so the value also travels once in the `POST /auth/login` / `GET /auth/csrf-token` JSON response body for the SPA to cache in memory (SDR-007 amendment); the cookie remains what the server actually validates against.
